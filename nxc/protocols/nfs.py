...
class nfs(connection):
    ...
    def get_root_handles(self, mount_fh):
        ...
        if filesystem in [FileID.ext, FileID.unknown]:
            root_handles.extend((bytes(fh[:3] + b"\x02" + fh[4:4 + fh_fsid_len] + b"\x02\x00\x00\x00" + b"\x00\x00\x00\x00" + b"\x02\x00\x00\x00"), bytes(fh[:3] + b"\x02" + fh[4:4 + fh_fsid_len] + b"\x80\x00\x00\x00" + b"\x00\x00\x00\x00" + b"\x80\x00\x00\x00")))
